...
Regenerate with:
  python3.12 scripts/generate_spec_tests.py
"""
...
REFERENCE_TEST_DIR = Path(__file__).parent.parent / "reference_test"
...
if sys.version_info >= (3, 14):
    raise SystemExit(
        "generate_spec_tests.py is not stable on Python 3.14; use Python 3.12 or 3.13."
    )
...
def value_to_json(value: Any) -> Any:
    ...
    if isinstance(value, complex):
        real = value.real
        imag = value.imag
        if math.isnan(real) or math.isinf(real) or math.isnan(imag) or math.isinf(imag):
            raise ValueError("unsupported complex")
...
def collect_cpython_cases() -> Tuple[List[str], List[List[str]]]:
    base_dir = CPYTHON_TEST_DIR
    if not base_dir.exists():
        if REFERENCE_TEST_DIR.exists():
            base_dir = REFERENCE_TEST_DIR
        else:
            raise FileNotFoundError(
                "CPython tests not found. Clone CPython into "
                f"{CPYTHON_TEST_DIR} or provide {REFERENCE_TEST_DIR}."
            )
    expr_cases: List[str] = []
    program_cases: List[List[str]] = []
    file_count = 0
    for path in sorted(base_dir.rglob("test_*.py")):
